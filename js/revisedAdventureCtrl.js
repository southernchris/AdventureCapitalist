function adventureCtrl (constants) {
    var vm = this;
        
    vm.accOpen = [false, false, false, false, false, false];
    vm.accOpen2 = [false, false];
    vm.rain = {};
    vm.clearAfter = [false, false];
    vm.compare = false;
    vm.earth = {};
    vm.fillBefore = [false, false];
    vm.filterTime = {'days': null, 'hours': null, 'minutes': null, 'percentage': null};
    vm.illionsArray = constants.illionsArr.slice(1);
    vm.mars = {};
    vm.moon = {};
    vm.raw = false;
    vm.ref = vm.rain;
    vm.reverse = true;
    vm.selectAll = [false, false, false, false];
    vm.showUpdate = false;
    vm.sortIndex = 2;
    ...
    function loadExportedJson(str) {
        ...
        for (k in planets) {
            if (obj.hasOwnProperty(planets[k])) {
                vm.fullyResetPlanet(vm[planets[k]]);
                for (i in obj[planets[k]].levels) {
                    if (obj[planets[k]].levels.hasOwnProperty(i)) {
                        for (j = 0; j < vm[planets[k]].investments.length; j++) {
                            if (vm[planets[k]].investments[j][0] === i) {
                                vm[planets[k]].investments[j][1] = obj[planets[k]].levels[i];
                                break;
                            }
                        }
                    }
                }
                vm[planets[k]].numAngels = obj[planets[k]].numAngels;
                vm[planets[k]].viewNumAngels = vm[planets[k]].numAngels;
                for (i = 0; i < obj[planets[k]].upgradeIndexUpTo; i++) {
                    vm[planets[k]].cashUpgrades[i][vm[planets[k]].cashUpgrades[i].length - 1] = true;
                }
                for (i = 0; i < obj[planets[k]].angelUpgradeIndexUpTo; i++) {
                    vm[planets[k]].angelUpgrades[i][vm[planets[k]].angelUpgrades[i].length - 1] = true;
                }
                for (i = 0; i < obj[planets[k]].upgradeIndexBonus.length; i++) {
                    vm[planets[k]].cashUpgrades[obj[planets[k]].upgradeIndexBonus[i]][vm[planets[k]].cashUpgrades[obj[planets[k]].upgradeIndexBonus[i]].length - 1] = true;
                }
                for (i = 0; i < obj[planets[k]].angelUpgradeIndexBonus.length; i++) {
                    vm[planets[k]].angelUpgrades[obj[planets[k]].angelUpgradeIndexBonus[i]][vm[planets[k]].angelUpgrades[obj[planets[k]].angelUpgradeIndexBonus[i]].length - 1] = true;
                }
                for (i = 0; i < obj[planets[k]].managersBought.length; i++) {
                    vm[planets[k]].managerUpgrades[Math.floor(obj[planets[k]].managersBought[i] / 2)][obj[planets[k]].managersBought[i] % 2][1] = true;
                }
                vm[planets[k]].noSingles = obj[planets[k]].noSingles || false;
                vm[planets[k]].noTens = obj[planets[k]].noTens || false;
                vm[planets[k]].triples = obj[planets[k]].triples;
                vm[planets[k]].flux = obj[planets[k]].flux;
                vm[planets[k]].bonusAngelEffectiveness = obj[planets[k]].bonusAngelEffectiveness;
                vm[planets[k]].bonusMultiplier = obj[planets[k]].bonusMultiplier;
                if (angular.isDefined(obj[planets[k]].megaTicket)) {
                    for (i = 0; i < obj[planets[k]].megaTicket.length; i++) {
                        vm[planets[k]].investments[obj[planets[k]].megaTicket[i]][2] = true;
                    }
                }
            }
            vm.calc(vm[planets[k]]);
        }
        $scope.$digest();
    }
    }

angular.module('adventureApp').controller('adventureController', ['$document', '$filter', '$scope', 'constants', 'calculate', 'earthConstants', 'moonConstants', 'marsConstants', 'rainConstants', function($document, $filter, $scope, constants, calculate, earthConstants, moonConstants, marsConstants, rainConstants) {
